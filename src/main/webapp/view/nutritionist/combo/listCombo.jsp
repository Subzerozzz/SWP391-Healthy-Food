...
<body class="body">
                  ...
                  <div class="flex items-center justify-between gap10 flex-wrap">
                    <div class="wg-filter flex-grow">
                      <form class="form-search" action="${pageContext.request.contextPath}/managerCombo" method="get">
                        <fieldset class="name">
                          <input type="text" placeholder="Search here..." class="name" name="find" tabindex="2"
                            value="${param.find}" required="">
                    ...
                    <a class="tf-button style-1 w208"
                      href="${pageContext.request.contextPath}/managerCombo?action=add"><i class="icon-plus"></i>Add
                      new</a>
                  </div>
                      ...
                      <c:forEach items="${listCombo}" var="cb">
                        <li class="product-item gap14">
                          <div class="flex items-center justify-between gap20 flex-grow">
                            <div class="name">
                              <a href="${pageContext.request.contextPath}/managerCombo?action=viewComboFoodDetail&id=${cb.comboId}"
                                class="body-title-2">${cb.comboName == null ? 'null' : cb.comboName}</a>
                            </div>
                            ...
                            <div class="list-icon-function" style="width: 130px">
                              <div class="item eye">
                                <a
                                  href="${pageContext.request.contextPath}/managerCombo?action=viewComboFoodDetail&id=${cb.comboId}"><i
                                    class="icon-eye"></i></a>

                              </div>
                              <div class="item edit">
                                <a
                                  href="${pageContext.request.contextPath}/managerCombo?action=edit&comboId=${cb.comboId}"><i
                                    class="icon-edit-3"></i></a>

                              </div>

                              <label class="switch" title="${cb.status eq 'active' ? 'inactive' : 'active'}">
                                <input type="checkbox" <c:if test='${cb.status eq "active"}'>checked</c:if>
                                onchange="location.href =
                                '${pageContext.request.contextPath}/managerCombo?action=${cb.status eq 'active' ?
                                'inactive' : 'active'}&comboId=${cb.comboId}'" />
                        ...
                        <c:choose>
                          ...
                          <c:otherwise>
                            ...
                            <c:if test="${currentPage < totalPages}">
                              <li>
                                <a
                                  href="?action=${param.action}&page=${totalPages}&pageSize=${pageSize}">${totalPages}</a>
                              </li>
                            </c:if>
                          </c:otherwise>
                        </c:choose>
  ...
  <c:if test="${not empty sessionScope.toastMessage}">
    <script>
      iziToast.$ {
        ...
        title: '${sessionScope.toastType == "success" ? "Th�nh c�ng" : "That bai"}',
  ...
  <style>
    /* CSS cho bi?u t??ng m?t (view) m�u v�ng */
    .list-icon-function .item.eye i {
      color: #f1c40f;
      /* M�u v�ng t??i */
    }

    /* CSS cho bi?u t??ng edit (b�t) m�u ?? */
