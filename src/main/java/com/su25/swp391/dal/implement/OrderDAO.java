...
//        order.setQuantity(rs.getString("quantity"));
    ...
    public List<Integer> findOrderIdsByUserId(int userId) {
    List<Integer> orderIds = new ArrayList<>();
    String sql = "SELECT order_id FROM orders WHERE user_id = ?";

    try {
        connection = getConnection(); // hoặc dùng DataSource nếu có
        statement = connection.prepareStatement(sql);
        statement.setInt(1, userId);
        ...
        System.out.println("Error finding order IDs by user ID: " + ex.getMessage());
    } finally {
    ...
    public static void main(String[] args) {
        OrderDAO orderDAO = new OrderDAO(); // hoặc dùng Singleton nếu có

        int userId = 24;
        String status = null; // có thể test với "pending"
        String paymentMethod = null; // có thể test với "Vnpay" hoặc "cod"
        int page = 1;
        int pageSize = 10;

        List<Order> orders = orderDAO.findOrdersByUserId(userId, status, paymentMethod, page, pageSize);

        if (orders.isEmpty()) {
            System.out.println(" Không tìm thấy đơn hàng nào cho userId: " + userId);
        } else {
            System.out.println("Tìm thấy " + orders.size() + " đơn hàng cho userId: " + userId);
            for (Order order : orders) {
                System.out.println("Order ID: " + order.getOrderId());
                System.out.println("Total: " + order.getTotal());
                System.out.println("Payment Method: " + order.getPaymentMethod());
                System.out.println("Status: " + order.getStatus());
                System.out.println("Username: " + order.getStatus());
                System.out.println("-------------------------------");
            }
        }
    }
